...
%  Using convolution to implement the filter
% 27Mar2018

%% The main function
function top88_conv2_function(nelx,nely,volfrac,penal,rmin,ft)
...
% Using convolution to make filter
[dy,dx] = meshgrid(-ceil(rmin)+1:ceil(rmin)-1, ...
-ceil(rmin)+1:ceil(rmin)-1);

% ceil function: Round toward positive infinity
% Syntax: Y=ceil(X)
% Y = ceil(X) rounds each element of X to the nearest integer greater than 
% or equal to that element.

% meshgrid function: 2-D grids
% Syntax: [X,Y] = meshgrid(x,y)
% [X,Y] = meshgrid(x,y) returns 2-D grid coordinates based on the coordinates 
% contained in vectors x and y. X is a matrix where each row is a copy of x, 
% and Y is a matrix where each column is a copy of y. The grid represented by 
% the coordinates X and Y has length(y) rows and length(x) columns.

h = max(0,rmin-sqrt(dx.^2+dy.^2));
% h is the non-zero part of the filter kernel
Hs = conv2(ones(nely,nelx),h,'same');
% Hs is the sum in the denominator on the right hand side of x(k,l)
...
while change > 0.01
  ...
  while (l2-l1)/(l1+l2) > 1e-3 % Stop condition is specified in relative terms 
    ...
    if ft == 1
      xPhys = xnew;
    elseif ft == 2
      xPhys = conv2(xnew,h,'same')./Hs; % density filtering
    end
