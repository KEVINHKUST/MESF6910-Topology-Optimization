...
%% The main function
function top88_Black_and_white_solution(nelx,nely,volfrac,penal,rmin,ft)
...
% xTide: intermediate density  
% xPhys gives the equation 21
% Project xTilde to xPhys(xPhys = 0 or 1, in theory)
...
while change > 0.01
  ...
  % wrt the physical densities
  % eqution 5  
  dv = ones(nely,nelx);
  % Assumption: each element has unit volume
  ...
  elseif ft == 3
      ...
      % dx = partial xe bar partial xe tilde
      % equation 23
      
      % The sensitivities of the function(compliance and volume) with
      % respect to the intermediate densities xe tilde are obtained by means
      % the chain rule
      % partial c partial xe tilede = partial c partial xe bar times
      % partial xe bar partial xe tilde
      
      dc(:) = H*(dc(:).*dx(:)./Hs);
      % xe bar: physical density
      % xe tilde:Intermediate density
      % To calculate the partial c partial xe tilde
      % dc = H / Hs * (partial c partial xe bar) * (partial xe bar partial xe tilde)
      % partial c partial xe bar = H*(dc(:)./Hs)
      % H / Hs implement the sensitivity calculation
      
      dv(:) = H*(dv(:).*dx(:)./Hs);
      % partial v partial xe tilde
      % dv = H / Hs * (partial v partial xe bar) * (partial xe bar partial xe tilde)
  end
  ...
  while (l2-l1)/(l1+l2) > 1e-3 % Stop condition is specified in relative terms 
    ...
    elseif ft == 3
        xTilde(:) = (H*xnew(:))./Hs;
        % equation 9 , xTilde equals the x after density filtering
        xPhys = 1-exp(-beta*xTilde) + xTilde*exp(-beta);
    end
    if sum(xPhys(:)) > volfrac*nelx*nely 
        % Rely upon to correct the violation of the volume constraint 
        % Lagrange multiplier lmid is 
